...
class processor(object):
    ...
    def execute_patch(self, patch_img1, patch_img2, patch_img3, mask_img, patch_count, save_dir, name):
        resize_image1 = cv2.resize(patch_img1, (self.patch_size, self.patch_size), cv2.INTER_AREA)
        ...
        resize_mask = cv2.resize(mask_img, (self.patch_size, self.patch_size), cv2.INTER_NEAREST)
    ...
    def get_patch(self, magnification=100):
        slide_name = self.slide_path.split('/')[-1]
        print(f'{slide_name}')
            ...
            for y in y_seq:
                for x in x_seq:
                    ...
                    if self.get_ratio_mask(sum_patch) >= 0.3:
                        patch_count += 1 
                        self.execute_patch(img_patch1, img_patch2, img_patch3, sum_patch, patch_count, save_dir=save_patch, name=name)